...
- **Dedupe `slot::enumerate` calls per `acquire`** — currently `find_same_sha_holder`, `count_occupying_in_group`, and `acquirable_ns` each call `enumerate` separately → 3 pool-dir reads per acquire. Pass an `&[SlotEntry]` once. Minor savings (each call is fast post-`worktree_gitdir` optimization), but cleaner.
...
- **Test slot's submodule working tree post-land.** `tests/smoke.rs` covers slot→main propagation (5 cases) but no test asserts the *slot's* submodule HEAD is at the expected gitlink after land — exactly the regression surface for the §Land-flow step-10 fix. Repro pattern: 2-slot fixture, slot A bumps submodule, lands; slot B (branched before A landed) does unrelated work, lands; assert `git -C <slot_b>/<sub> rev-parse HEAD == <new_gitlink>` and `git -C <slot_b> status --porcelain` is empty.
- **Test merge-commit parent order after parallel land.** Land's parent-reorder block (`bin/wt:425-447`) has no integration coverage. Repro: 2-slot fixture, slot A commits + lands (ff main), slot B (branched earlier) commits + lands → forces a real 3-way merge. Assert main's tip has parents `(prev_main, slot_b_tip)` in that order, and `git log --first-parent main` stays on mainline (doesn't dive into slot_b's history).
...
- **`assert!(out.status.success(), "...stderr={}", String::from_utf8_lossy(&out.stderr))` repeats ~50× across `tests/smoke.rs`.** A `assert_ok(out: &Output, ctx: &str)` helper would unify these. Pre-existing; surfaced by simplify multi-agent review during the merge-parent-reorder add.
