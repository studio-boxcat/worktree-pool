...
Reviewers ran over the perf changes (`a504730`..`c17a34b`). Real bug got fixed inline; these are deferred follow-ups.
...
- **`worktree_rename` mid-failure recovery** — if `fs::rename` succeeds but `git worktree repair` or `rewrite_submodule_worktrees` fails, the slot is left partially repaired with no idempotent retry path. `release` calls `worktree_rename` again on un-rename, which tries to reverse-substitute path segments that don't match the leftover state — silent corruption of un-touched submodule pointers. Mitigation: at the start of every recycled-slot acquire, run `git worktree repair` defensively and re-walk `modules/**/config` to rebuild from the absolute slot path. Or: make `rewrite_slot_segment` accept "either old OR new is fine" semantics so retries are idempotent.
