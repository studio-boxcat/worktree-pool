...
- **`session new` resume should verify slot health, not just dir presence** — `cmd_new` (`bin/worktree-pool-session`) treats `[ -d "$POOL_ROOT/$name" ]` as "resume this slot". This is sound for the normal held-slot case but skips lock-presence verification, so the documented crash-recovery state (CLAUDE.md §Crash recovery — rename succeeded but lock-write failed) would be silently picked up as if held; the cleanup trap on shell exit then runs a `release` whose lock-read fails and emits noise. Harden by requiring lock presence (e.g. `worktree-pool --pool <key> inspect --name <name>` exits 0) before treating dir-exists as resumable, and surfacing a distinct error with the right manual-clear hint (`git -C <source> worktree remove --force <slot>`) for the no-lock case. Low priority — crash-recovery state is rare and the symptom is cosmetic noise, not data loss.
