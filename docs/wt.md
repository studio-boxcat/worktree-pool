...
8. **Pre-stage submodule sync, then auto-commit dirty tracked work.** Before staging, sweep every submodule listed in `.gitmodules` (skip those without a `.git`): compare index gitlink (`git ls-files -s`) against working HEAD. If working HEAD is an ancestor of the index gitlink — the classic post-merge-without-`--recurse-submodules` state, where stock `git merge` advanced the recorded gitlink but left the submodule working dir at the pre-merge SHA — run `git -c protocol.file.allow=always submodule update -- <sub>` to bring the working dir forward. Without this sync, the `git add -u` below would silently regress the gitlink to match the stale working dir, undoing the merge (step 10's preflight would then catch its own corruption with a confusing "diverged" message). The reverse direction (working HEAD ahead of index) is a legitimate forward bump — leave it for `git add -u` to stage. True divergence (no shared ancestor) refuses with a recovery hint. `protocol.file.allow=always` is required because pool slots' submodule clones use `file://` origins pointing at the source's `.git/modules/...` — git's default blocks the file transport. Then auto-commit dirty tracked work (if any remains after the sync) with the supplied message. Refuses if dirty *and* no message. `wip` shorthand → `WIP via land`.
