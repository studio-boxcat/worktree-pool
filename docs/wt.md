...
11. **Submodule ancestry pre-flight.** For each top-level submodule whose gitlink moved between `<main_before>` and slot HEAD, fetch the slot's branch into main's submodule clone, then `git -C <main>/<sub> merge-base --is-ancestor <main-clone-HEAD> <slot-clone-HEAD>`. Any failure aggregates into a single die with per-sub rebase hint (`cd <main>/<sub> && git rebase <slot-tip>`). Surfaces step 12's ff-only refusal early, before parent advance. Local fetches reuse objects step 12 will need.
