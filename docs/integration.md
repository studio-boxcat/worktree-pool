...
- **No reclaim on holder death.** A SIGKILL'd holder leaves the slot on its branch (held); operator notices via `ls` and runs `release`. (A crash *mid* acquire/release instead converges on its own — idle slots get recycled, a half-acquired held slot clears with one idempotent `release` — see [[lifecycle.md#crash-recovery]].)
